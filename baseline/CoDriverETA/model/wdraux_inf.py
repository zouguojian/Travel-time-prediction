# -- coding: utf-8 --
from baseline.CoDriverETA.model.lstm import *
from baseline.CoDriverETA.model.triplet_loss import batch_all_triplet_loss, batch_hard_triplet_loss

class WDRAUXClass(object):
    """
    ...
    def inference(self, X=None, feature_inds=[], keep_prob=0.0, speed=None, ave_s=None):
        '''
        forward propagation
        :param X:
        :param feature_inds:
        ...
        # triple loss
        with tf.variable_scope('TRIPLE_LOSS', reuse=False):
            speed_embedding = tf.gather(v, feature_inds)[:, :1] # [N, dim]
            print(speed_embedding.shape, ave_s.shape)
            aux_loss = batch_hard_triplet_loss(labels= tf.squeeze(ave_s, axis=1),
                                               embeddings= tf.squeeze(speed_embedding, axis=1),
                                               margin=0.01,
                                               squared=True)
        # MLP
