...
class DeepFM(object):
    ...
    def inference(self, X=None, feature_inds=[], keep_prob=0.0, hiddens=None, speed=None):
        '''
        forward propagation
        :param X:
        :param feature_inds:
        ...
        v = tf.get_variable(name='dense_matrix',shape=[self.p, self.k],dtype=tf.float32,initializer=tf.initializers.truncated_normal())
                                                   # p 总的轨迹数据元素长度
        ...
        if self.hp.model_name == 'DNN':
            ...
            with tf.variable_scope('FM', reuse=False):
                ...
                x_1 = tf.expand_dims(tf.matmul(X, v), axis=2)
                x_2 = tf.tile(x_1, [1, 1, self.emb_size])
                interaction = tf.matmul(x_2, tf.transpose(tf.matmul(X, v), [1, 0]))
                interaction = tf.reduce_mean(interaction, axis=2)
        ...
        with tf.variable_scope('separate_and_sum_trajectory',reuse=False):
            ...
            x_common = tf.reduce_sum(x_common[:, 2:, :], axis=1, keep_dims=True) # (N, 1, 64) departure 时间
            ...
            x_trajectory_separate = tf.concat([x_trajectory_distances, x_trajectory_route_id], axis=-1) # (N, 5, 2 * 64)
            ...
            speed = tf.reduce_mean(speed, axis=2, keep_dims=True)
            x_trajectory_separate = tf.add(speed, x_trajectory_separate)

            # interaction = tf.expand_dims(interaction,axis=1)
            # interaction = tf.expand_dims(interaction,axis=1)
            # interaction = tf.tile(interaction, [1, self.trajectory_length, 1, 1])  # (N, 5, 1, 64)
            # x_trajectory_separate = tf.add(interaction, x_trajectory_separate)

            # x_trajectory_separate = tf.squeeze(x_trajectory_separate, axis=2)
            # S = SpatialTransformer(self.hp)
            # x_trajectory_separate = S.encoder(x_trajectory_separate)
            # x_trajectory_separate = tf.expand_dims(x_trajectory_separate, axis=2)
            ...
            if self.hp.model_name=='Hatt':
                ...
                x_trajectory_separate = T.encoder(hiddens=hiddens[:, :],
                                                  hidden=x_trajectory_separate, num_heads=2, num_blocks=1)
            ...
            # x_trajectory_sum = tf.add(interaction, x_trajectory_sum)
        ...
        with tf.variable_scope('spatio_temporal_deep_fm_fusion', reuse=False):
            ...
            y_out_1 = tf.layers.dense(x_1, units=1)
            y_out_1 = tf.squeeze(y_out_1, axis=-1)  # (N, 5)

            x_2 = tf.layers.dense(x_trajectory_sum, units=self.k, activation=tf.nn.relu)
            y_out_2 = tf.layers.dense(x_2, units=1)

            if self.hp.model_name != 'DNN':
                # y_out_2 = tf.add_n([self.linear_terms, y_out_2])
                ...
                y_out_2 = tf.layers.dense(y_out_2, units=1)
