...
from baseline.CoDriverETA.model.wdraux_inf import WDRAUXClass
from baseline.CoDriverETA.model.data_next import DataClass
from baseline.CoDriverETA.model.utils import construct_feed_dict, one_hot_concatenation, metric, FC
...
class Model(object):
    ...
    def initial_placeholder(self):
        ...
        self.placeholders = {
            ...
            'ave_s': tf.placeholder(tf.float32, shape=[None, 1], name='average_speed'),
    ...
    def model(self):
        ...
        with tf.variable_scope(name_or_scope='trajectory_model'):
            WDRAUXModel = WDRAUXClass(self.hp)
            self.y_wdr, self.aux_loss = WDRAUXModel.inference(X=self.placeholders['feature_tra'],
                                                               feature_inds=self.placeholders['feature_inds'],
                                                               keep_prob=self.placeholders['dropout'],
                                                               speed=speed[:, self.input_length - 1],
                                                               ave_s=self.placeholders['ave_s'])
        ...
        self.loss_1 = tf.reduce_mean(mape)

        self.loss = 0.62 * self.loss_1 + 0.38 * self.aux_loss
    ...
    def run_epoch(self):
        ...
        for i in range(int(iterate.shape_tra[0] * self.divide_ratio) * self.epoch // self.batch_size):
            ...
            element_index, separate_trajectory_time, total_time, trajectory_inds, ave_s, _,_,_,_ = self.sess.run(train_next)
    ...
    def evaluate(self):
        ...
        for i in range(int(iterate_test.shape_tra[0] * (1 - self.hp.divide_ratio) - 15 * (
            ...
            x_s, week, day, hour, minute, label_s, \
            vehicle_id, vehicle_type, start_week, start_day, start_hour, start_minute, start_second, distances, route_id, \
            element_index, separate_trajectory_time, total_time, trajectory_inds, ave_s, dates, vehicle_id_str, vehicle_type_int, whether_app = self.sess.run(test_next)
            x_s = np.reshape(x_s, [-1, self.input_length, self.site_num, self.feature_s])
            week = np.reshape(week, [-1, self.site_num])
            day = np.reshape(day, [-1, self.site_num])
            hour = np.reshape(hour, [-1, self.site_num])
